...
FText FAudioSection::GetSectionTitle() const
{
	...
	return NSLOCTEXT("FFrameInputsSection", "NoFrameInputsTitleName", "No Frame Inputs");
}
...
FFrameInputsTrackEditor::FFrameInputsTrackEditor(TSharedRef<ISequencer> InSequencer)
	: FMovieSceneTrackEditor(InSequencer)
{
}

FFrameInputsTrackEditor::~FFrameInputsTrackEditor()
{
}
...
void FFrameInputsTrackEditor::BuildAddTrackMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.AddMenuEntry(
		LOCTEXT("AddTrack", "Frame Inputs Track"),
		LOCTEXT("AddTooltip", "Adds a sequence of frame inputs for TAS playback."),
		FSlateIcon(FEditorStyle::GetStyleSetName(), "Sequencer.Tracks.Audio"),
		FUIAction(
			FExecuteAction::CreateRaw(this, &FFrameInputsTrackEditor::HandleAddAudioTrackMenuEntryExecute)
		)
	);
}

void FFrameInputsTrackEditor::BuildObjectBindingTrackMenu(FMenuBuilder& MenuBuilder, const TArray<FGuid>& ObjectBindings, const UClass* ObjectClass)
{
	if (ObjectClass != nullptr && ObjectClass->IsChildOf(AActor::StaticClass()))
	{
		MenuBuilder.AddSubMenu(
			LOCTEXT("AddAttachedFrameInputsTrack", "FrameInputs"),
			LOCTEXT("AddAttachedFrameInputsTooltip", "Adds a frame input track attached to the object."),
			FNewMenuDelegate::CreateSP(this, &FFrameInputsTrackEditor::HandleAddAttachedAudioTrackMenuEntryExecute, ObjectBindings));
	}
}
...
const FSlateBrush* FFrameInputsTrackEditor::GetIconBrush() const
{
	return FEditorStyle::GetBrush("Sequencer.Tracks.Audio");
}

bool FFrameInputsTrackEditor::IsResizable(UMovieSceneTrack* InTrack) const
{
	return true;
}

void FFrameInputsTrackEditor::Resize(float NewSize, UMovieSceneTrack* InTrack)
{
...
bool FFrameInputsTrackEditor::OnAllowDrop(const FDragDropEvent& DragDropEvent, FSequencerDragDropParams& DragDropParams)
{
...
FReply FFrameInputsTrackEditor::OnDrop(const FDragDropEvent& DragDropEvent, const FSequencerDragDropParams& DragDropParams)
{
	if (!DragDropParams.Track.IsValid() || !DragDropParams.Track.Get()->IsA(UMovieSceneAudioTrack::StaticClass()))
	{
	...
	for (const FAssetData& AssetData : DragDropOp->GetAssets())
	{
		...
		if (Sound)
		{
			if (DragDropParams.TargetObjectGuid.IsValid())
			{
				...
				AnimatablePropertyChanged(FOnKeyProperty::CreateRaw(this, &FFrameInputsTrackEditor::AddNewAttachedSound, Sound, AudioTrack, OutObjects));
			}
			else
			{
				AnimatablePropertyChanged(FOnKeyProperty::CreateRaw(this, &FFrameInputsTrackEditor::AddNewMasterSound, Sound, AudioTrack, DragDropParams.RowIndex));
			}
...
TSharedPtr<SWidget> FFrameInputsTrackEditor::BuildOutlinerEditWidget(const FGuid& ObjectBinding, UMovieSceneTrack* Track, const FBuildEditWidgetParams& Params)
{
	...
	return SNew(SHorizontalBox)
		...
		[
			FSequencerUtilities::MakeAddButton(LOCTEXT("FrameInputsText", "Frame Inputs"), FOnGetContent::CreateSP(this, &FFrameInputsTrackEditor::BuildAudioSubMenu, FOnAssetSelected::CreateRaw(this, &FFrameInputsTrackEditor::OnAudioAssetSelected, Track), FOnAssetEnterPressed::CreateRaw(this, &FFrameInputsTrackEditor::OnAudioAssetEnterPressed, Track)), Params.NodeIsHovered, GetSequencer())
		];
}

bool FFrameInputsTrackEditor::HandleAssetAdded(UObject* Asset, const FGuid& TargetObjectGuid)
{
	if (Asset->IsA<USoundBase>())
	{
		...
		const FScopedTransaction Transaction(LOCTEXT("AddFrameInputs_Transaction", "Add Frame Inputs"));
		...
		if (TargetObjectGuid.IsValid())
		{
			...
			AnimatablePropertyChanged(FOnKeyProperty::CreateRaw(this, &FFrameInputsTrackEditor::AddNewAttachedSound, Sound, DummyTrack, OutObjects));
		}
		else
		{
			int32 RowIndex = INDEX_NONE;
			AnimatablePropertyChanged(FOnKeyProperty::CreateRaw(this, &FFrameInputsTrackEditor::AddNewMasterSound, Sound, DummyTrack, RowIndex));
		}
...
FKeyPropertyResult FFrameInputsTrackEditor::AddNewMasterSound(FFrameNumber KeyTime, USoundBase* Sound, UMovieSceneAudioTrack* AudioTrack, int32 RowIndex)
{
	...
	if (ensure(AudioTrack))
	{
		...
		{
			AudioTrack->SetDisplayName(LOCTEXT("FrameInputsTrackName", "Frame Inputs"));
...
FKeyPropertyResult FFrameInputsTrackEditor::AddNewAttachedSound(FFrameNumber KeyTime, USoundBase* Sound, UMovieSceneAudioTrack* AudioTrack, TArray<TWeakObjectPtr<UObject>> ObjectsToAttachTo)
{
	FKeyPropertyResult KeyPropertyResult;
	...
	for (int32 ObjectIndex = 0; ObjectIndex < ObjectsToAttachTo.Num(); ++ObjectIndex)
	{
		...
		if (ObjectHandle.IsValid())
		{
			...
			if (ensure(AudioTrack))
			{
				...
				AudioTrack->SetDisplayName(LOCTEXT("FrameInputsTrackName", "FrameInputs"));
				KeyPropertyResult.bTrackModified = true;
				KeyPropertyResult.SectionsCreated.Add(NewSection);
...
/* FFrameInputsTrackEditor callbacks
 *****************************************************************************/

void FFrameInputsTrackEditor::HandleAddAudioTrackMenuEntryExecute()
{
	UMovieScene* FocusedMovieScene = GetFocusedMovieScene();
	...
	const FScopedTransaction Transaction(NSLOCTEXT("Sequencer", "AddFrameInputsTrack_Transaction", "Add FrameInputs Track"));
	FocusedMovieScene->Modify();

	auto NewTrack = FocusedMovieScene->AddMasterTrack<UMovieSceneAudioTrack>();
	ensure(NewTrack);

	NewTrack->SetDisplayName(LOCTEXT("FrameInputsTrackName", "Frame Inputs"));
...
void FFrameInputsTrackEditor::HandleAddAttachedAudioTrackMenuEntryExecute(FMenuBuilder& MenuBuilder, TArray<FGuid> ObjectBindings)
{
	MenuBuilder.AddWidget(BuildAudioSubMenu(FOnAssetSelected::CreateRaw(this, &FFrameInputsTrackEditor::OnAttachedAudioAssetSelected, ObjectBindings), FOnAssetEnterPressed::CreateRaw(this, &FFrameInputsTrackEditor::OnAttachedAudioEnterPressed, ObjectBindings)), FText::GetEmpty(), true);
}


TSharedRef<SWidget> FFrameInputsTrackEditor::BuildAudioSubMenu(FOnAssetSelected OnAssetSelected, FOnAssetEnterPressed OnAssetEnterPressed)
{
...
void FFrameInputsTrackEditor::OnAudioAssetSelected(const FAssetData& AssetData, UMovieSceneTrack* Track)
{
...
void FFrameInputsTrackEditor::OnAudioAssetEnterPressed(const TArray<FAssetData>& AssetData, UMovieSceneTrack* Track)
{
	if (AssetData.Num() > 0)
	{
...
void FFrameInputsTrackEditor::OnAttachedAudioAssetSelected(const FAssetData& AssetData, TArray<FGuid> ObjectBindings)
{
	FSlateApplication::Get().DismissAllMenus();
